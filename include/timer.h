/* 
 * Periodic threads using POSIX timers and signals.
...
void setup_alarm_sig(sigset_t *alarm_sig) {
    sigemptyset(alarm_sig);
    for (int i = SIGRTMIN; i <= SIGRTMAX; i++)
        sigaddset(alarm_sig, i);                 		 //adicionar cada sinal à variável alarm_sig
    sigprocmask(SIG_BLOCK, alarm_sig, NULL);			//bloquear todos os sinais contidos em alarm_sig
}
